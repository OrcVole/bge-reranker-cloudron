#!/bin/bash
#
# Secret-scan and anonymity sweep. Run before any push (the release gate).
#
...
# every file except the .git dir, the scratchpad, the local-only list, and the local-only docs.
if git rev-parse --git-dir >/dev/null 2>&1; then
  ...
  mapfile -t files < <(find . -type f \
    ...
    -not -name STATUS.md -not -name RECON.md -not -name FOUNDATION-PROMPT.md)
fi
...
for tok in githubtoken.txt palladiumtoken.txt forgejotoken.txt; do
